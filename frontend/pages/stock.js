...
export default function Stock() {
  ...
  const tickerResizeObserver = useRef();
  ...
  const api_url = `https://eodhistoricaldata.com/api/eod/AAPL.US?from=${start}&'${current}&period=d&fmt=json&api_token=63bf6ecd8c46c5.53082791`;
  const host = 'http://localhost:5000';
  
  useEffect(() => {
    let ticker_chart = createChart(tickerContainerRef.current, {
      width: tickerContainerRef.current.clientWidth,
      height: 500, 
      layout: {
    ...
    setChart(ticker_chart);

    let candleSeries = ticker_chart.addCandlestickSeries({
      upColor: "#26a69a",
      downColor: "#ef5350",
    ...
    let volumeSeries = ticker_chart.addHistogramSeries({
      color: "#182233",
      lineWidth: 2,
      priceFormat: {
        type: "volume"
    ...
    let lineSeries = ticker_chart.addLineSeries({
      color: '#2962FF',
      lineWidth: 2,
    ...
    ticker_chart.subscribeCrosshairMove(param => {
      if (
  ...
  useEffect(() => {
    if(tickerData.length > 0 && chart) {
      candleSeries.setData(tickerData);
      lineSeries.setData(convertToLineData(tickerData));
      volumeSeries.setData(tickerData);
      tickerResizeObserver.current = new ResizeObserver(entries => {
      ...
      tickerResizeObserver.current.observe(tickerContainerRef.current);
      return () => tickerResizeObserver.current.disconnect();
    }
  }, [tickerData, chart]);
    ...
    try {
      switch (range) {
        case 1:
          load && removeLoading()
          loading();
          var res = await axios.get(`${host}/interval_1m`);
          if(res.status == 200) {
            setTickerData(res.data.rows ? convertToTickerData(res.data.rows) : []);
            removeLoading()
          }
          break;
        case 2:
          load && removeLoading()
          loading();
          var res = await axios.get(`${host}/interval_5m`);
          if(res.status == 200) {
            setTickerData(res.data.rows ? convertToTickerData(res.data.rows) : []);
            removeLoading()
          }
        ...
        case 3:
          load && removeLoading()
          loading();
          var res = await axios.get(`${host}/interval_1d`);
          if(res.status == 200) {
            setTickerData(res.data.rows ? convertToTickerData(res.data.rows) : []);
            removeLoading()
          }
  ...
  const setDownload = (e) => {
    if(e.target.value == "excel") {
      ...
      const ws = XLSX.utils.json_to_sheet(tickerData);
  ...
  const showMax = (e) => {
    ...
    for (var i=0 ; i<tickerData.length ; i++) {
        if (max == null || parseFloat(tickerData[i].high) > parseFloat(max.high)) {
          max = tickerData[i]; index = tickerData.length-i
        }
    }
    chart.timeScale().scrollToPosition(-index, true);
  }

  const convertToTickerData = (data) => {
    return data.map(o=>{
      return {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div className={styles.flex}>
          ...
          <span className={styles.btn} onClick={(e) => { e.preventDefault(); setRange(2) }}>5 Days</span>
          <span className={styles.btn} onClick={(e) => { e.preventDefault(); setRange(3) }}>1 Month</span>
